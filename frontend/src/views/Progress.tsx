...
export function Progress({
  ...
  function phaseProgress(phaseIdx: number): number {
    ...
    const [start, end] =
      phaseIdx === 1 ? [0, 38] : phaseIdx === 2 ? [38, 72] : [72, 95]
  ...
  const topicLabel =
    kitMeta?.lesson_topic || formData?.lesson_topic || "Bài giảng"
      ...
      <div className="lk-fade-up mb-7 text-center">
        {complete ? (
          <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-[12.5px] font-medium text-emerald-700 ring-1 ring-inset ring-emerald-100">
            <Check width={13} height={13} strokeWidth={2.6} /> Hoàn thành · Đang
            mở bài giảng...
          </span>
        ) : failed ? (
        ...
        <p className="mt-1 text-[13.5px] text-slate-500">
          {duration} phút · {grade ? `Lớp ${grade} · ` : ""}CEFR {supportLevel}{" "}
          · Song ngữ Anh–Việt
        </p>
      </div>
