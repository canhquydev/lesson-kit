...
export function getLessons(subject: string, grade: string) {
  return api.get<LessonItem[]>(
    `/lessons?subject=${encodeURIComponent(subject)}&grade=${encodeURIComponent(grade)}`,
  )
}
...
export function regenerateAllStale(kitId: string) {
  return api.post<RegenerateAllStaleResult>(
    `/lesson-kit/${kitId}/regenerate-all-stale`,
  )
}
